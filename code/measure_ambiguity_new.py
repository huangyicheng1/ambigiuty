...
class AmbiguityMeasurement:
    ...
    def calculate_daily_cdfs(self):
        ...
        for date, group in self.df.groupby(pd.Grouper(freq='D')):
            if not group.empty and len(group) > 10:
    ...
    def calculate_monthly_ambiguity(self):
        ...
        for month, month_group in self.daily_cdfs_df.groupby(
            ...
            if not month_group.empty and len(month_group) > 5:
                # 提取当月所有天的CDF和CDF差值
    ...
    def plot_results(self):
        ...
        ax1.plot(self.monthly_ambiguity_df['month'], 
                ...
                color='blue',
                markersize=4)
        ax1.set_title('Monthly Market Ambiguity', fontsize=12, pad=10)
        ax1.set_xlabel('Time')
        ...
        ax2.scatter(self.monthly_ambiguity_df['avg_sigma'], 
                   ...
                   alpha=0.6,
                   color='red')
        ax2.set_title('Ambiguity vs Return Volatility', fontsize=12, pad=10)
        ax2.set_xlabel('Average Daily Volatility')
        ...
        ax3.hist(self.monthly_ambiguity_df['ambiguity'], 
                ...
                color='green',
                edgecolor='black')
        ax3.set_title('Ambiguity Distribution', fontsize=12, pad=10)
